...
int IsAdmin(const user *User) {
   ...
   return UserLevel(User) >= GroupLevel(SearchGroup(ADMIN_GROUP));
}
...
int IsUpperEqUser(const user *User1, const user *User2) {

   if (User1 == NULL) {
      Error("IsUpperEqUser(): User1 = NULL");
      return FALSE;
   } else if (User1->Type != USER) {
      Error("IsUpperEqUser(): \"%s\" is not a user",User1->Id);
      return FALSE;
   } else if (User2 == NULL) {
      Error("IsUpperEqUser(): User2 = NULL");
      return FALSE;
   } else if (User2->Type != USER) {
      Error("IsUpperEqUser(): \"%s\" is not a user",User2->Id);
      return FALSE;
   }
